...
void update_backup_log(const char *logfile, log_t *logs) {
    // Ouvrir le fichier log en mode lecture pour obtenir l'existant
    FILE *file = fopen(logfile, "r");
    if (!file) {
        perror("Erreur lors de l'ouverture du fichier log en lecture");
        return;
    }

    // Lire tout le contenu du fichier log existant
    char buffer[1024];
    char content[1024 * 10];  // Un buffer assez grand pour contenir le fichier entier (à adapter selon la taille du log)
    content[0] = '\0';  // Initialiser la chaîne vide

    while (fgets(buffer, sizeof(buffer), file)) {
        strcat(content, buffer);  // Ajouter chaque ligne du fichier à content
    }

    fclose(file);  // Fermer après lecture

    // Ouvrir le fichier log en mode ajout (append) pour ajouter la nouvelle entrée à la fin
    ...
    log_element *new_entry = logs->head;
    if (!new_entry) {
        fprintf(stderr, "Aucune entrée à ajouter au log.\n");
        fclose(file);
        return;
    }

    // Ajouter la nouvelle entrée à la fin du fichier log
    fprintf(file, "%s;%s;%s\n", new_entry->path, new_entry->date, new_entry->md5);

    fclose(file);  // Fermer après l'écriture
}
...
int compare_file_with_backup_log(const char *path, log_t *logs, const char *backup_name, const char *logfile) {
    ...
    if (S_ISREG(file_stat.st_mode)) {
        ...
        new_entry->path = malloc(PATH_MAX * sizeof(char));  
        new_entry->date = malloc(20 * sizeof(char));  
        ...
        snprintf(new_entry->path, PATH_MAX, "%s/%s", basename(backup_name), file_basename); 
        snprintf(new_entry->date, 20, "%s", formatted_date);  // Date de modification du fichier au format YYYY-MM-DD:HH:MM:SS

        strncpy(new_entry->md5, file_md5_str, MD5_DIGEST_LENGTH * 2 + 1);  



        // Ajouter l'élément à la tête de la liste chainée
    ...
    if (S_ISDIR(file_stat.st_mode)) {
        ...
        char *dir_basename = basename(path);  
